# Determining the optimal sliding window size for proposed CNN model with Cross-Validation
# Allows to get more accurate estimation of the length of sliding window
########################################################################################################################
...
for w in range(2, 152, 2):  # sliding window
    ...
    for fold in range(n_folds):
        print("window size w = " + str(w) + " runnig fold " + str(fold) + " buccal...")
        ...
        def train(epoch):
            ...
            valAccuracy = torch.sum(
                (torch.argmax(output_val, dim=1).type(torch.FloatTensor) == y_val).detach()).float() / y_val.shape[0]
            trainAccuracy = torch.sum(
                (torch.argmax(output_train, dim=1).type(torch.FloatTensor) == y_train).detach()).float() / \
                            y_train.shape[0]
        ...
        testAccuracyPerFold.append(float(testAccuracy))
        trainLossPerFold.append(float(train_losses[-1]))
        valLossPerFold.append(float(val_losses[-1]))
        trainAccuracyPerFold.append(float(train_accuracies[-1]))
        valAccuracyPerFold.append(float(val_accuracies[-1]))

    # append the metrics:
    endTrainLossTotal.append(trainLossPerFold)
    endValLossTotal.append(valLossPerFold)
    endTrainAccuracyTotal.append(trainAccuracyPerFold)
    endValAccuracyTotal.append(valAccuracyPerFold)
    totalTestAccuracy.append(testAccuracyPerFold)

########################################################################################################################
# SAVE
########################################################################################################################
endTrainLossTotalBuccal_fn = ('RESULTS/endTrainLossTotalBuccal.p')
with open(endTrainLossTotalBuccal_fn, 'wb') as fp:
    pickle.dump(endTrainLossTotal, fp)

endValLossTotal_fn = ('RESULTS/endValLossTotalBuccal.p')
with open(endValLossTotal_fn, 'wb') as fp:
    pickle.dump(endValLossTotal, fp)

endTrainAccuracyTotal_fn = ('RESULTS/endTrainAccuracyTotalBuccal.p')
with open(endTrainAccuracyTotal_fn, 'wb') as fp:
    pickle.dump(endTrainAccuracyTotal, fp)

endValAccuracyTotal_fn = ('RESULTS/endValAccuracyTotalBuccal.p')
with open(endValAccuracyTotal_fn, 'wb') as fp:
    pickle.dump(endValAccuracyTotal, fp)

totalTestAccuracy_fn = ('RESULTS/totalTestAccuracyBuccal.p')
...
x = np.arange(2, 2 * len(endValLossTotal) + 1, 2)
plt.plot(x, np.mean(list(endTrainLossTotal), axis=1))
plt.plot(x, np.mean(list(endValLossTotal), axis=1))
...
plt.ylabel("Loss")
plt.tight_layout()
plt.savefig('FIGURES/endLossMetrics_window_4_features_CV.pdf')

plt.figure(figsize=(14, 5))
x = np.arange(2, 2 * len(endValAccuracyTotal) + 1, 2)
plt.plot(x, np.mean(list(endTrainAccuracyTotal), axis=1))
plt.plot(x, np.mean(list(endValAccuracyTotal), axis=1))
plt.plot(x, np.mean(list(totalTestAccuracy), axis=1))
plt.title('Train accuracy metrics after 100 epochs and test accuracy vs window size, averaged over 12 folds')
...
plt.ylim([0, 1])
plt.tight_layout()
plt.savefig('FIGURES/endAccuracyMetrics_window_4_features_CV.pdf')
